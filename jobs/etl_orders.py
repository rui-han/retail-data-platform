...
def normalize_orders(df: DataFrame) -> DataFrame:
    return (
        df.select(*[F.trim(F.col(c)).alias(c) if t == "string" else F.col(c) for c, t in df.dtypes])
...
def run():
    ...
    valid = cleaned.filter(F.col(PRIMARY_KEY).isNotNull()).dropDuplicates([PRIMARY_KEY])
    invalid = cleaned.filter(F.col(PRIMARY_KEY).isNull()).withColumn("dq_reason", F.lit("missing_order_id"))

    write_parquet(valid, storage.silver_path("stg_orders"))
    write_parquet(invalid, storage.silver_path("stg_orders_quarantine"))

    logger.info("Orders staging done. valid=%s invalid=%s", valid.count(), invalid.count())
